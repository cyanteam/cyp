...
cyp 管理软件包的安装、卸载、更新和依赖关系。包格式为体积较小的二进制包格式(~~简称 zip~~)，里面包含 TOML 配置和资源文件，没有任何二进制魔法。

- 小包直接从服务器(~~cf白嫖~~)下载，大包从 GitHub 仓库获取
- 支持多架构（macOS ARM/Intel、Linux x64、Windows x64，理论上能安装Node.js的系统应该都可以使用）
